	...
	<div class="xv-member-card" data-user-id="{$data.user.user_id}">
		...
		<xf:if is="$xf.visitor.canFollowUser($data.user)">
			<a href="{{ link('members/follow', $data.user) }}"
				class="xv-member-follow-btn js-followBtn"
...
// Update JS modification to check follow state from a single AJAX call
$jsReplace = <<<'JSTEMPLATE'
<xf:css src="member.less" />
<xf:set var="$followingJson" value="{{ json_encode($xf.visitor.Profile.following) }}" />
<xf:js>
// XenVibe Following Data
window.xvFollowing = {$followingJson};
</xf:js>
<xf:js>
