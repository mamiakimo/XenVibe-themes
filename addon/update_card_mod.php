...
<xf:macro id="overview_row" arg-data="!">
	<div class="xv-member-card">
		...
		<xf:set var="$userTitle" value="{$data.user.custom_title ?: $data.user.user_title}" />
		<xf:if is="$userTitle">
			<span class="xv-member-badge">{$userTitle}</span>
		</xf:if>
		...
		<a href="{{ link('members/follow', $data.user) }}" class="xv-member-follow-btn">
...
if ($result) {
    echo "Updated xv_member_card_redesign modification successfully!\n";
    ...
    if ($template) {
        echo "Recompiling member_notable template...\n";
        $template->getBehavior('XF:DevOutputWritable')->setOption('write_dev_output', false);
        $template->save();
        echo "Done!\n";
    }
} else {
    echo "No rows updated - modification may not exist\n";
}
